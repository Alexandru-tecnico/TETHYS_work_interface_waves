...
void BoundaryCondition::YClosedFreeSlip(Fluid2D& fluid_class){
	int nx=fluid_class.SizeX(); //should be Nx no??
	for (int i=0; i < nx; i++){
		int j_top;
		j_top = TopEdge[i];
		int j_bot;
		...
		fluid_class.FlxX[bottom] = fluid_class.FlxX[top - nx];
		fluid_class.FlxY[bottom] = Slope * fluid_class.FlxX[top - nx];// Isn't this off? Should be FlxY[top-nx]?
		fluid_class.FlxX[top] = fluid_class.FlxX[bottom + nx];
		fluid_class.FlxY[top] = -1.0f * Slope * fluid_class.FlxX[bottom + nx];

	}
}
void BoundaryCondition::YClosedNoSlip(Fluid2D& fluid_class){// what does this do- flux at bottom and top vanishes
	int nx=fluid_class.SizeX();
	for (int i=0; i < nx; i++) {
		int j_top;
		j_top = TopEdge[i];
