...
void TethysBase::CloseHdf5File() const{
    ...
    /****/
    Dataspacecycfrq -> close();
    Dataspaceoddvis -> close();
...
TethysBase::~TethysBase(){
	if(Hdf5FileOpen) {
        ...
        delete Dataspacecycfrq;
        delete Dataspaceoddvis;
		if (RANK == 2) {
			delete GrpVelY;
			delete DataspaceVelY;
			delete DataspaceVelSndMid;
            /****/
            delete DataspacecycfrqMid;
            delete DataspaceoddvisMid;
			delete DataspaceTmp;
		}
		delete Hdf5File;
	}
...
TethysBase::TethysBase(int size_nx, int size_ny, int dimension){
	...
	if(RANK==1){
        ...
        /****/
        Dataspacecycfrq = new DataSpace(RANK, dimsf);
        Dataspaceoddvis = new DataSpace(RANK, dimsf);
	}
	if(RANK==2){
		hsize_t dimsf[2];
        ...
        /****/
        DataspacecycfrqMid = new DataSpace(RANK, dimsf);
        DataspaceoddvisMid = new DataSpace(RANK, dimsf);
	}
