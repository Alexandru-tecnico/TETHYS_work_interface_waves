...
SetUpParameters::SetUpParameters(float sound, float fermi, float coll, float visco, float odd_visco, float cyclo,
								 float thermal_diff, int mode, float aspect) {/* ok parece que vai buscar aqui--*/
	SizeX=101;
	SizeY=101;
	SoundVelocity = sound;
	FermiVelocity = fermi;
...
void SetUpParameters::DefineGeometry() {
	int sampling=151;//CHANGED HERE!!-- supostamente devia alterar todos...hm-- 1.5x seems to work...
	if(AspectRatio>1.0f){
