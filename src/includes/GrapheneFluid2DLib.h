 ...
 * */
class GrapheneFluid2D : public Fluid2D{//hm-- MAYBE HERE IT IS!!
protected:
	/*!
...
public :
	explicit GrapheneFluid2D(SetUpParameters &input_parameters);
		...
		float XMomentumSource(float n, float flx_x, float flx_y, float mass, float s,GridPoint p,int k)override; ///< velocity X component equation (momentum equation) st
		float YMomentumSource(float n, float flx_x, float flx_y, float mass, float s, GridPoint p,int k)override; ///< velocity y component equation (momentum equation) st
		...
		float YMomentumFluxY(GridPoint p, char side ) override; ///< velocity Y component equation (momentum equation) conserved flux Y component
                          ...
                          // Central Algorithm for solving the hyperbolic conservation law

        void Richtmyer_ung();// For ungated potential
        void GetPhi(float phi[], float *n, int Sx,int Sy,float dx,float dy);  // Phi for UNGATED
         /************/
        float XMomentumFluxX_ung(GridPoint p, char side);
        float YMomentumFluxY_ung(GridPoint p, char side);
